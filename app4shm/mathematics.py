...
def interpolate_data_stream(data_stream: list[Data]):
    ...
    while start_me < t_end:
        t_interval.append(start_me)
        start_me += TIME_INCREMENT
    inter_x = interpn(data_times, data_x, t_interval, INTERPOLATION_TYPE)
    inter_y = interpn(data_times, data_y, t_interval, INTERPOLATION_TYPE)
