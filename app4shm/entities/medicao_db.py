...
def showColx(s):
    with GROUPLOCK:
        return SESSION.query(Medicao).filter(Medicao.usernameGroup == s).all()

def delCol(idDelete: str):
    with GROUPLOCK:
        SESSION.query(Medicao).filter(Medicao.usernameGroup == idDelete).delete()
        SESSION.commit()